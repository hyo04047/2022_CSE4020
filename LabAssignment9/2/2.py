...
def log(R):
    ...
    if tr == 3:
        ...
        theta = np.pi
        v = np.array([R[0, 2], R[1, 2], R[2, 2] + 1] / np.sqrt(2 * (1 + R[2, 2])))
        if(v == np.array([0, 0, 0])):
            v = np.array([R[0, 1], R[1, 1] + 1, R[2, 1]] / np.sqrt(2 * (1 + R[1, 1])))
        if(v == np.array([0, 0, 0])):
            v = np.array([R[0, 0] + 1, R[1, 0], R[2, 0]] / np.sqrt(2 * (1 + R[0, 0])))
    else:
...
def render(ang):
    ...
    glEnable(GL_LIGHT0)
    # rescale normal vectors after transformation and before lighting to have unit length
